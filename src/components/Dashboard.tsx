...
export default function Dashboard({ userId, onLogout }: DashboardProps) {
  ...
  if (showCamera) {
    return <CameraCapture userId={userId} onClose={() => setShowCamera(false)} onFoodLogged={handleFoodLogged} userGoal={profile.goal} />;
  }

  if (showBarcodeScanner) {
    return <BarcodeScanner userId={userId} onClose={() => setShowBarcodeScanner(false)} onFoodLogged={handleFoodLogged} />;
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-emerald-50 to-teal-50">
      <div className="max-w-4xl mx-auto p-3 sm:p-4 pb-28 sm:pb-24">
        <div className="flex justify-between items-center mb-4 sm:mb-6 pt-2 sm:pt-4">
          <div>
            <h1 className="text-2xl sm:text-4xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">NutriTrack</h1>
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-3 sm:gap-0">
            ...
            <div className="flex flex-col xs:flex-row items-stretch xs:items-center gap-2">
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-3 py-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white rounded-xl transition-all text-xs sm:text-sm font-medium shadow-lg hover:shadow-emerald-500/50"
                title="Switch between bulk and cut"
              >
                <ArrowLeftRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                <span className="whitespace-nowrap">{profile.goal === 'bulk' ? 'Switch to Cut' : 'Switch to Bulk'}</span>
              </button>
            </div>
          </div>

          <div className="space-y-4 sm:space-y-5">
            {/* Calories Card - Clean Display with Orange Theme */}
            <div className="bg-gradient-to-br from-orange-50 via-orange-100 to-orange-200 rounded-2xl p-6 sm:p-8 shadow-sm border border-orange-200/50">
              <div className="flex items-start justify-between">
                <div className="flex flex-col">
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-4xl sm:text-5xl font-bold text-orange-900">{todayCalories}</span>
                    <span className="text-2xl sm:text-3xl text-orange-600/70 font-medium">/{profile.daily_calories}</span>
                  </div>
                  <div className="text-sm sm:text-base text-orange-700 mt-2">Calories eaten</div>
                </div>
                <div className="relative w-16 h-16 sm:w-20 sm:h-20">
                  <svg className="w-full h-full transform -rotate-90">
                    ...
                    <circle
                      ...
                      strokeDashoffset={`${2 * Math.PI * 28 * (1 - Math.min(todayCalories / profile.daily_calories, 1))}`}
            ...
            {/* Macros Grid - Three Minimal Cards */}
            <div className="grid grid-cols-3 gap-3 sm:gap-4">
              {/* Protein Card */}
              <div className="bg-gradient-to-br from-red-50 via-red-100 to-rose-200 rounded-2xl p-4 sm:p-5 shadow-sm border border-red-200/50">
                <div className="flex flex-col items-start">
                  <div className="flex items-baseline gap-1 mb-1">
                    <span className="text-xl sm:text-2xl font-bold text-red-900">{Math.round(todayProtein)}</span>
                    <span className="text-sm sm:text-base text-red-600/70 font-medium">/{profile.daily_protein}g</span>
                  </div>
                  <div className="text-xs sm:text-sm text-red-700 mb-1">Protein eaten</div>
                  <div className="text-xs text-red-600 mb-3">{Math.max(0, profile.daily_protein - todayProtein).toFixed(0)}g remaining</div>
                  <div className="relative w-12 h-12 sm:w-14 sm:h-14 mt-auto">
                    <svg className="w-full h-full transform -rotate-90">
                      <circle
                        cx="50%"
                        cy="50%"
                      ...
                      <circle
                        ...
                        strokeDashoffset={`${2 * Math.PI * 20 * (1 - Math.min(todayProtein / profile.daily_protein, 1))}`}
                        className="text-red-600 transition-all duration-500"
                        strokeLinecap="round"
                      />
                    </svg>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <Beef className="w-5 h-5 sm:w-6 sm:h-6 text-red-600" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Carbs Card */}
              <div className="bg-gradient-to-br from-amber-50 via-yellow-100 to-amber-200 rounded-2xl p-4 sm:p-5 shadow-sm border border-amber-200/50">
                <div className="flex flex-col items-start">
                  <div className="flex items-baseline gap-1 mb-1">
                    <span className="text-xl sm:text-2xl font-bold text-amber-900">{Math.round(todayCarbs)}</span>
                    <span className="text-sm sm:text-base text-amber-600/70 font-medium">/{profile.daily_carbs}g</span>
                  </div>
                  <div className="text-xs sm:text-sm text-amber-700 mb-1">Carbs eaten</div>
                  <div className="text-xs text-amber-600 mb-3">{Math.max(0, profile.daily_carbs - todayCarbs).toFixed(0)}g remaining</div>
                  <div className="relative w-12 h-12 sm:w-14 sm:h-14 mt-auto">
                    <svg className="w-full h-full transform -rotate-90">
                      <circle
                        cx="50%"
                      ...
                      <circle
                        ...
                        fill="none"
                        strokeDasharray={`${2 * Math.PI * 20}`}
                        strokeDashoffset={`${2 * Math.PI * 20 * (1 - Math.min(todayCarbs / profile.daily_carbs, 1))}`}
              ...
              {/* Fats Card */}
              <div className="bg-gradient-to-br from-blue-50 via-blue-100 to-blue-200 rounded-2xl p-4 sm:p-5 shadow-sm border border-blue-200/50">
                <div className="flex flex-col items-start">
                  <div className="flex items-baseline gap-1 mb-1">
                    <span className="text-xl sm:text-2xl font-bold text-blue-900">{Math.round(todayFats)}</span>
                    <span className="text-sm sm:text-base text-blue-600/70 font-medium">/{profile.daily_fats}g</span>
                  </div>
                  <div className="text-xs sm:text-sm text-blue-700 mb-1">Fats eaten</div>
                  <div className="text-xs text-blue-600 mb-3">{Math.max(0, profile.daily_fats - todayFats).toFixed(0)}g remaining</div>
                  <div className="relative w-12 h-12 sm:w-14 sm:h-14 mt-auto">
                    <svg className="w-full h-full transform -rotate-90">
                      <circle
                        cx="50%"
                      ...
                      <circle
                        ...
                        fill="none"
                        strokeDasharray={`${2 * Math.PI * 20}`}
                        strokeDashoffset={`${2 * Math.PI * 20 * (1 - Math.min(todayFats / profile.daily_fats, 1))}`}
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="border-t border-gray-200 my-4 pt-4">
                <button
                  ...
                  className="w-full flex items-center gap-3 p-4 bg-red-50 hover:bg-red-100 rounded-xl transition-all"
                >
